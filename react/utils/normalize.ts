// https://github.com/vtex-apps/product-summary/blob/cd511311e69dcffc438250303b3ceb9d541a1f61/react/ProductSummaryListWithoutQuery.tsx#L56
import type { ProductTypes } from 'vtex.product-context'

type ItemImage = NonNullable<ProductTypes.Item['images']>[number]
type ItemReference = NonNullable<ProductTypes.Item['referenceId']>[number]

type NormalizedSeller = ProductTypes.Seller

export type NormalizedSKU = ProductTypes.Item & {
  seller: NormalizedSeller
  referenceId: ItemReference
  image: ItemImage
  images: ItemImage[]
}

export type NormalizedProductSummary = ProductTypes.Product & {
  sku?: NormalizedSKU
}
...
const defaultImage = { imageUrl: '', imageLabel: '' } as ItemImage
const defaultReference = { Value: '' } as ItemReference
const defaultSeller = {} as NormalizedSeller
 ...
 * - The mapCatalogProductToProductSummary function is not exported by VTEX as a public API.
Since VTEX does not provide official support to extend or import it directly, we need to clone the function into our project.
 * */
